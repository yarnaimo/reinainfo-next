...
export const ScheduleDetailContent = memo<Props>(
  ({ children, schedule: s, tickets, compact, ...props }) => {
    ...
    const Divider_ = !compact && (s.formattedDate.parts || decodedTickets) && (
      <Block compact={compact}></Block>
    )

    const Parts_ = s.formattedDate.parts && (!compact || !decodedTickets) && (
      <Block compact={compact} css={{ ...margin({ x: -3 }), flexWrap: 'wrap' }}>
        {s.formattedDate.parts.map((p, i) => (
          <PartChip
            key={i}
            label={p.name ? `${p.name} | ${p.timesOfPart}` : p.timesOfPart}
          ></PartChip>
        ))}
      </Block>
    )
      ...
      <Block compact={compact} column>
        {decodedTickets.map((t, i) => (
          ...
          >
            ...
            >
              {t.closed ? (
                <s css={{ color: color.black(0.4) }}>{t.text}</s>
              ) : (
                t.text
              )}
            </Liquid>
    ...
    const Actions_ = !compact && (
      <Block compact={compact} css={{ ...margin({ top: -2, bottom: -6 }) }}>
        <Liquid></Liquid>
        {/* <TweetButton
